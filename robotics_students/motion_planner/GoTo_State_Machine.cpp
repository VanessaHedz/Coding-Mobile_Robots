...
int go_to(Inputs inputs)
{
 ...
 coord coord_robot;  // **********************************************************          COORDENATES OF THE ROBOT !
 coord coord_dest;   // **********************************************************   COORDENATES OF THE LIGHT SOURCE !
 int flg;
 int debug=0;
 ...
 int flg_result;
 float angle_light; // ****************************************************************** ANGLE OF THE LIHGT SOURCE !
 ...
 while(flg == 0){
	...
	if(selection == 1){
		...
		printf("avoidance destination behavior: angle  %f distance %f\n",DistTheta.angle,DistTheta.distance);
                ...
                // state_machine_students in ../state_machines/state_machine_students.h
...
//Behavior 7
                ...
                // state_machine_students in ../state_machines/state_machine_students.h
                DistTheta = Bug1_students(observations, quantized_attraction, quantized_intensity,state,&next_state,inputs.Mag_Advance,inputs.max_angle, inputs.num_sensors, angle_light, coord_robot, coord_dest);
